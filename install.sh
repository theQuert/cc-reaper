...
echo "  guard:          runaway-MCP reaper every 10 min (SIGTERMs one shared MCP server 60+ min old that is"
echo "                  still >=80% CPU on a re-check; never an application, dev server or its process group)"
