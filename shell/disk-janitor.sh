...
# Config defaults
# ---------------------------------------------------------------------------
# Operator overrides. The template writes `: "${NAME:=value}"`, so an explicit environment
...
_cc_dj_clean() {
  ...
  if ! command -v docker >/dev/null 2>&1; then
    ...
    # The one prune verb here, and it reaches build cache only. BuildKit never prunes a
    # record a running build holds, and until=168h keeps anything used within a week, so
    # a CI build keeps its cache without the runner-drain proof this replaced - which no
    # producer ever issued, behind a runner-name gate the always-up runners never cleared.
    _cc_dj_clean_target "docker builder cache unused 168h+" \
