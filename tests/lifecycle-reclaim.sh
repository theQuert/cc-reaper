...
CC_REAPER_ROOT="$SANDBOX/root" CC_REAPER_TEST_LOG="$log" CC_REAPER_LIFECYCLE_LOG="$SANDBOX/lifecycle.log" \
...
grep -q 'start stage=staging-complete' "$SANDBOX/lifecycle.log"
grep -q 'worktree=complete status=0' "$SANDBOX/lifecycle.log"
grep -q 'disk=complete status=0' "$SANDBOX/lifecycle.log"
...
if CC_REAPER_ROOT="$SANDBOX/root" CC_REAPER_TEST_LOG="$log" CC_REAPER_LIFECYCLE_LOG="$SANDBOX/lifecycle.log" \
...
grep -q 'disk=complete status=7' "$SANDBOX/lifecycle.log"
