...
| `CC_DJ_TRIM_WORKTREES` | `1` | environment | `0` disables low-disk delegation from `disk-janitor --clean` to the worktree-preserving regenerable trim. |
...
~/.cc-reaper/worktree-janitor.sh --repo "$HOME/GitHub/example"
# Under disk pressure, report ignored package/build trees without removing the worktree:
~/.cc-reaper/worktree-janitor.sh --repo "$HOME/GitHub/example" --trim-regenerable
# The disk janitor is the pressure-gated caller for the apply form:
~/.cc-reaper/worktree-janitor.sh --repo "$HOME/GitHub/example" --trim-regenerable --apply
...
Safety boundaries (hard): never touches user data (`~/Documents`, `~/Downloads`, `~/Desktop`) unless an operator explicitly adds a repository root there, runs no `docker prune` except the weekly `docker builder prune --force --filter until=168h`, never removes an image, never removes **any** volume, worktree cleanup never kills processes or deletes branches, and a hook payload whose own cwd cannot be established turns that SessionEnd sweep into a report.

What the docker step *does* remove: build cache the daemon reports unused for at least 168 hours, and nothing else. BuildKit never prunes a record a running build holds, so a CI build keeps its cache. Dangling images are reported, not removed: on a shared host an untagged image is another user's build. The step once ran `docker system prune -af`, which reaches every unused tagged image including ones that take hours to rebuild; that is not "rebuildable-only" and is gone. **Unused images survive.**
