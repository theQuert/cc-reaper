...
condition that failed. A value of `CC_WJ_ARCHIVE_MAX_BYTES` that is not a decimal integer SHALL
...
`<CC_WJ_ARCHIVE_DIR>/<repository directory name>/<worktree directory name>-<UTC time>/<the file's relative path>`.
