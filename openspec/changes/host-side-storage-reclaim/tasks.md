...
- [ ] 1.1 Validate this change strictly; delete the superseded `lifecycle-adapters` change; move the
...
- [ ] 2.1 Red then green: an executed run whose PATH lacks the tool directory still resolves `gh`
...
- [ ] 2.2 Red then green: an apply sweep that finds a live holder logs the deferral and exits 0;
  a lock that cannot be taken still exits non-zero.

## 3. Disk janitor

- [ ] 3.1 `--clean` runs exactly `docker builder prune --force --filter until=168h` when the daemon
...
- [ ] 3.2 Remove `--orbstack-clean`, drain-proof validation, protected-container filters,
...
- [ ] 4.1 A drop of at least 10 GB against a 25 to 45 minute old sample logs `ALERT:disk-drop` and
...
- [ ] 5.1 Isolate the absent-root janitor tests from real harness roots (552ea76); document
