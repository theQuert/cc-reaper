...
- **`claude-guard`'s runaway phase signals one re-checked MCP server.** It signalled the selected PID's whole process group - a shared MCP server started by a Claude CLI took the CLI, its stream-json subagents and sibling servers with it - on one CPU sample, reading process age as time spent hot, and it selected applications and dev servers: on one host its LaunchAgent signalled ChatGPT.app and cmux.app, the terminal the sessions ran in. Now it selects a process only when the process itself is a known shared MCP server - its executable, or what a package runner or interpreter runs, never a name somewhere in its arguments (a session whose `--settings` named `claude-mem` qualified) - and only when its CPU time is at least `CC_RUNAWAY_CPU` percent of a life of at least `CC_RUNAWAY_MIN` minutes, from a process listing that carries no argument text. It re-reads each PID after at least three seconds, signals only if the command is unchanged and still over `CC_RUNAWAY_CPU`, and signals that PID alone. A stall late in a long-lived server is diluted by the lifetime average and not reaped; `cc-monitor` still reports it. `_claude_pgid_kill` loses its `force_target` argument, and `cc-monitor` names `claude-guard` as the remedy only for what it would select.
- **`install.sh` sources the deployed copies, and never stops over the rc file.** Shell rc lines read `[ -r "$HOME/.cc-reaper/claude-cleanup.sh" ] && source ...` instead of naming the checkout the installer ran from, which under worktree-per-session was a task worktree that reclamation removed - every shell printed errors and the commands were missing. An update replaces lines in the old generated shape (printing them), removes them beside a current line or a hand-written one (which it prints and leaves as the only source line), adds the current line when the old one is commented out, and backs the rc file up before any change - never a file the run itself created - keeping its mode, ACL and extended attributes on a rewrite. A symlinked, hard-linked, unreadable or unwritable rc file is left alone with the change printed, and the installation completes.
...
- **`worktree-janitor` reclaims only landed, idle, unheld worktrees.** The previous gate (clean + no process cwd from a `pgrep` subset + not detached) classified a worktree REMOVABLE while a session was editing it through `git -C`, and removed checkouts whose work had never reached the base. Now: machine-wide `lsof` of every cwd **and every open file**, each bounded and each failing closed on an empty or failed scan; `CC_WJ_IDLE_HOURS` (default 6, a malformed value refuses the run); landing proven against a freshly fetched base by ancestry, `git merge-tree` content, or a merged PR at this exact head SHA whose merge commit is still on the base; a detached HEAD needs ancestry. Holders, contents and idleness are asked again immediately before each removal. Superseded: "an unpushed branch is still removable".
...
### Added
- **Stop hook: the documented blind spot.** A server started with `nohup ... &` from a tool call lives in that tool call's process group, not the session's, so this hook never sees it - and must not be taught to by pattern.
