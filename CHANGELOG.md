...
### Changed
- **The LaunchAgent monitor no longer selects by CPU.** Its runaway-CPU override killed any PPID=1 process at 80% CPU for three minutes whose command line contained `node`, `npx`, `mcp`, `bun`, `codex` or `claude` anywhere. All six kills in a live `monitor.log` (2026-09-10/11) were pytest runs and session scratchpad scripts - background work Claude Code reparents to launchd, carrying `/private/tmp/claude-501/` paths. Its age gate also failed open on an etime it could not parse. Stuck shared services stay with `claude-guard`'s runaway phase (protection classes, `CC_RUNAWAY_MIN`); orphaned unprotected MCP and agent processes are still reaped by the family sweep. `CC_RUNAWAY_ORPHAN_MIN_SEC` is gone.
- **`install.sh` sources the deployed copies.** Shell rc lines now read `[ -r "$HOME/.cc-reaper/claude-cleanup.sh" ] && source ...` instead of naming the checkout the installer ran from, which under worktree-per-session was a task worktree that reclamation removed - leaving every shell printing errors and the commands missing. An update repairs lines in the old generated shape after backing the rc file up, leaves other shapes alone and names them, and does not rename over a symlinked rc file.
