...
- **`archive:` lines in `.worktree-regenerable` let a hand-written record leave with its
  ...
  before the removal. `archive:` is asked before a plain declaration, and a directory a
  plain line discounts whole keeps its worktree while an `archive:` pattern may name a path
  inside it. An older janitor drops the line as a pattern that names no path.
